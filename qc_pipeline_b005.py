"""
QC Pipeline Validator — B005 Full Pipeline
==========================================
Reference: data/health-insurance-claim/synthetic data/claim_B005_full_pipeline.json
Output: qc_report_b005.md

Scenario: Emergency claim — HALTS at Node 2 (DUPLICATE_CLAIM).
           Nodes 3–6 are skipped.

Usage:
    .venv\\Scripts\\python.exe qc_pipeline_b005.py
"""
...
def main():
    ...
    print("=" * 60)
    print(f"  QC Pipeline B005 — {run_ts}")
...
""" + "\n".join(md_table_row(status_icon(r[0]), r[1], str(r[2])[:120], str(r[3])[:120], r[4]) for r in n1_cmp) + "\n\n---\n")
    ...
    print(f"    policy_verified={n2_ok}  verification_failure={n2_actual.get('verification_failure')}  | PASS={pass_c} FAIL={fail_c}" + (f" | ERR: {n2_err}" if n2_err else ""))

    sections.append(f"""
## Stage 2 — Policy Verification

**Endpoint:** `POST /verification/process`  
**Status:** {"❌ DUPLICATE_CLAIM (EXPECTED for B005)" if not n2_ok else "✅ Verified (UNEXPECTED for B005)"}  
...
""" + "\n".join(md_table_row(status_icon(r[0]), r[1], str(r[2])[:120], str(r[3])[:120], r[4]) for r in n2_cmp) + "\n\n---\n")
    ...
    # ── Pipeline halts here for B005 ─────────────────────────────────
    if not n2_ok:
        print(f"\n    ⛔ Pipeline halted at Node 2 — policy_verified=false, verification_failure={n2_actual.get('verification_failure')} (EXPECTED for B005)")
        print("    Nodes 3–6 skipped as designed.")
...
"""
    ...
    header = f"""# QC Report — B005 Full Pipeline
**Scenario:** {EXPECTED.get('_scenario', 'B005')}  
