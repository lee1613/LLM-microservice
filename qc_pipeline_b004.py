"""
QC Pipeline Validator — B004 Full Pipeline
==========================================
Reference: data/health-insurance-claim/synthetic data/claim_B004_full_pipeline.json
Output: qc_report_b004.md (detailed intermediate I/O + comparison against expected)

Scenario: Surgical (Appendectomy), COMP-HEALTH-BRONZE, Non-Panel, APPROVED.
           Full 6-stage pipeline through to disbursement.

Usage:
    .venv\\Scripts\\python.exe qc_pipeline_b004.py
"""
...
EXPECTED_FILE = Path(r"data\health-insurance-claim\synthetic data\claim_B004_full_pipeline.json")
DOCS_DIR = Path(r"data\health-insurance-claim\synthetic data\documents")
REPORT_FILE = Path("qc_report_b004.md")
HEADERS = {"Content-Type": "application/json"}
TIMEOUT = 120  # seconds per request (medical review can be slow)
...
def main():
    ...
    print("=" * 60)
    print(f"  QC Pipeline B004 — {run_ts}")
...
**Status:** {"✅ Verified" if n2_ok else "❌ Failed"}  
...
**Status:** {"✅ Approved" if n5_ok else "❌ Not Processed / Error"}  
...
""" + "\n".join(md_table_row(status_icon(r[0]), r[1], str(r[2])[:120], str(r[3])[:120], r[4]) for r in n5_cmp) + "\n\n---\n")
    ...
    # B004 uses provider_direct payment — provider is payee
    n6_payment = {
        "payment_mode": "provider_direct",
        "payee_name": "Novena Surgical and Orthopaedic Centre"
    }
...
**Status:** {"✅ Disbursed" if n6_ok else "❌ Error"}  
...
"""
    ...
    header = f"""# QC Report — B004 Full Pipeline
**Scenario:** {EXPECTED.get('_scenario', 'B004')}  
